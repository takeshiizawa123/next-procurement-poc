...
> **立替精算専用ページ**: `/expense/new` から簡易フォームで申請可能（承認・発注スキップ、証憑添付を強調した専用UI）
...
出張は「事前承認」ではなく「予約完了申請（事後承認）」方式を採用:

```
申請者: 各サービスで予約・決済（MFカード）
  ↓
申請者: /trip/new（Webフォーム）で予約完了を申請
  ↓ AIアシスタントで自動入力も可
Bot → #出張チャンネルに投稿（実額ベース）
  ↓
部門長: 事後承認（実額を確認して[承認]）
  ↓
月次: カード明細とMF経費で自動照合 → 仕訳
```

**設計意図**: 出張は事前に予約しないと料金が上がるため、先に予約→実額で申請→部門長が事後承認する方式が合理的。

### Webフォーム（/trip/new）の特徴

- **AIアシスタント**: 自然言語入力（例: 「4/21朝9時 大阪 新幹線 1泊」）→ Gemini解析 → フォーム自動入力 + 予約リンク生成
- **予約リンク生成**: Yahoo路線（時刻パラメータ付）/Googleマップ/じゃらん/楽天トラベル/スマートEX等のリンクを自動生成
- **複数行入力**: 交通費・宿泊費をサービス別に複数行で入力可能
- **PJコード検索**: MFマスタ337件から全件表示+1文字絞り込み
- **日当自動計算**: 日帰り1,000円 / 宿泊3,000円×日数
...
| レンタカー | トヨタレンタカー / 各社Web | MFカード | 法人カードを事前登録 |
| タイムズカー | タイムズカーアプリ | MFカード | 法人カードを事前登録 |
| その他 | 各サービス | MFカード | 利用交通手段欄に記入 |
...
### 出張統制ダッシュボード（/admin/trip-controls）

管理本部向けの出張経費統制画面。月次cronで自動更新。

**5つの統制機能:**

| 機能 | 内容 |
|------|------|
| 差異検知 | 申請額と実カード利用額の乖離を検出（HIGH/MEDIUM/LOW） |
| 未申請検出 | カード明細はあるが出張申請がない利用を検知 |
| 重複検出 | 同一ルート・同一期間の重複出張を検知 |
| 部門別コスト | 部門ごとの出張費用を集計、前月比較 |
| 個人ランキング | 従業員別の出張費用ランキング |
...
| タイミング | 処理 |
|-----------|------|
| 購買承認時 | カード払い → 従業員のカード下4桁を解決 → predicted_transactions テーブルに予測レコード作成 |
...
**前提**: 従業員マスタの card_last4, card_holder_name, mf_office_member_id が登録されていること
