...
**申請者がやること:**
- **証憑は購買申請システム（Slack or マイページ）に提出するだけでOK**
- 提出された証憑は **MFクラウド経費に自動転送** されます
- MFクラウド経費を開いて手動で証憑を添付する必要はありません
- カード明細との突合も管理本部がシステムで自動処理します
...
| 仕組み | 役割 | 申請者の操作 |
|--------|------|------------|
| **MFクラウド経費** | カード明細の自動取込 + 証憑の保管 | **なし**（自動転送される） |
| **購買申請システム** | 申請→承認→証憑の管理 + OCR照合 | **証憑を1回添付するだけ** |
| **週次突合バッチ** | 上記2つを自動照合。不一致を検知 | **なし**（自動） |

> まとめ: **証憑の提出先は購買申請システム（Slackスレッド or マイページ）の1箇所だけ**です。MFクラウド経費への転送は自動で行われるため、二重に提出する必要はありません。
...
照合結果:
  ✅ マッチ — カード明細と申請が一致。正常。
  🔴 未申請 — カード明細はあるが申請がない → 事後申請を依頼
  🟡 承認前 — カード利用日 < 承認日 → ルール違反の可能性
  🟠 金額不一致 — 明細と申請の金額が異なる → 確認が必要
```

**証憑との関係:**
- カード明細は「お金の動き」= MFクラウド経費が自動管理
- 証憑は「取引の証明」= 購買申請システムで申請者が添付
- 突合バッチが両者を自動的に突き合わせる
- **申請者に二重作業を求めない設計**（証憑提出は1箇所のみ）
...
納品書でも構いません。できれば**適格請求書（T+13桁の登録番号あり）** のものが税務上有利です。
