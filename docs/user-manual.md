...
```
読込 → CSVパース → 予測テーブル照合(Phase1) → スコア照合(Phase2) → 結果表示
```
...
| **要確認** | 候補が複数ある件 | 差異タグ（日付/金額/取引先名）を見て正しい明細を選択 → [これに確定] |
| **明細なし** | 申請はあるがカード明細にない件 | [来月もう一度照合] or [キャンセル済みかもしれない] |
| **未申請利用** | カード明細はあるが申請がない件 | [本人に確認] → Slack通知 or [経費で処理] |
