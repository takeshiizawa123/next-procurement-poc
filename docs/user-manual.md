...
- MFクラウド経費への証憑添付は不要です（管理本部が照合します）
- カード明細との突合は管理本部がシステムで自動処理します

**なぜ両方の仕組みがあるのか:**

| 仕組み | 役割 | 申請者の操作 |
|--------|------|------------|
| **MFクラウド経費** | カード明細の自動取込。「お金の動き」を記録 | **なし**（自動） |
| **購買申請システム** | 申請→承認→証憑の管理。「何を買ったか」を記録 | **証憑を添付** |
| **週次突合バッチ** | 上記2つを自動照合。不一致を検知 | **なし**（自動） |

> まとめ: **証憑の提出先は購買申請システム（Slackスレッド or マイページ）の1箇所だけ**です。MFクラウド経費に同じ書類を二重に提出する必要はありません。
