...
tbl("従来フローからの重要な変更点", ["項目", "旧フロー", "新フロー"],
    [
        ["発注", "管理本部が代行", "申請者が自分で実施"],
        ["カード決済", "管理本部カードで一括", "各社員のMFバーチャルカードで決済"],
        ["検収", "管理本部が確認", "申請者（または指定した検収者）が確認"],
...
tbl("入力項目", ["項目", "必須", "説明", "入力例"],
    [
        ...
        ["単価（税抜）", "★", "1個の金額", "45,000"],
        ["数量", "★", "個数", "1"],
        ["支払方法", "★", "カード/請求書/前払い/立替", "会社カード"],
        ["購入先", "★", "店舗/サイト名", "Amazon"],
        ["検収者", "", "物品確認者（未指定＝申請者）", "鈴木太郎"],
        ["購入目的", "★", "利用目的", "業務利用"],
        ["商品URL", "", "自動で情報取得", "https://..."],
    ])
...
bullets("承認された後の操作", [
    "カード払い:",
    ("DM: 「承認されました。カードで発注してください」", 1),
    ("MFビジネスカード（VISA）で購入 → [発注完了] ボタン", 1),
    ("3日後に自動リマインドDM（押し忘れ防止）", 1),
    "",
    "請求書払い:",
    ("DM: 「承認されました。発注してください」", 1),
    ("自分で発注 → [発注完了] ボタン", 1),
    ("届いた請求書をSlackスレッド or マイページから提出", 1),
    "",
    "VISAが使えない購入先 → 請求書払いを選択",
    "差戻し → DMで理由が届く → /purchase で再申請",
    "取消 → [取消] ボタン（発注前のみ）",
])
...
bullets("検収後のその他操作", [
    "返品が必要な場合:",
    ("検収済みの申請で [↩️ 返品] ボタンを押す", 1),
    ("確認ダイアログ → 「返品する」を選択", 1),
    ("管理本部に通知され、仕訳取消が処理される", 1),
    "",
    "10万円以上の購入:",
    ("検収完了時に管理本部へ「固定資産登録が必要」と自動通知", 1),
    ("申請者の追加操作は不要", 1),
], accent=CYAN)

tbl("支払方法別の提出書類", ["支払方法", "提出する書類", "提出後の処理"],
    [
        ["カード払い", "納品書 or 領収書", "自動仕訳ドラフト作成"],
        ["請求書払い", "請求書", "管理本部が支払処理"],
        ["立替（購入済）", "レシート or 領収書", "MF経費経由で給与精算"],
    ], accent=CYAN)
...
bullets("管理本部の日常", [
    ...
    "",
    "証憑・請求書の受付:",
    ("申請者が提出した証憑を確認（自動検知→OCR照合済み）", 1),
    ("請求書払い → 請求書を確認して支払処理（銀行振込等）", 1),
    "",
...
tbl("よくある質問", ["質問", "回答"],
    [
        ...
        ["金額に関係なく自分で発注？", "はい。部門長承認後は全件申請者が発注"],
