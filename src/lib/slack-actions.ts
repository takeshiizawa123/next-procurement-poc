/**
 * Slack インタラクティブアクションハンドラー
 *
 * ボタン押下、モーダル送信、スラッシュコマンド、Web操作同期
 * の全ハンドラーを提供する。
 */

import { WebClient } from "@slack/web-api";
import { getStatus, loadPurchaseDraft as _loadPurchaseDraft } from "./gas-client";
import { generatePrediction, isCardPayment } from "./prediction";
import { buildJournalFromPurchase, createJournal } from "./mf-accounting";
import {
  DEV_ADMIN_SLACK_ID,
  safeUpdateStatus,
  safeDmChannel,
...
export async function handleReturnSubmit(
  ...
  if (returnAmount > 0) {
    try {
      const { createJournal: createJ, resolveAccountCode, resolveTaxCode } = await import("@/lib/mf-accounting");
...
export async function handlePoTestCommand(
  ...
  const { buildNewRequestBlocks: buildBlocks } = await import("./slack-messages");
  await slackClient.chat.postMessage({
    channel: channelId,
    blocks: buildBlocks(requestInfo),
    text: `購買申請: ${poNumber}`,
  });
}
